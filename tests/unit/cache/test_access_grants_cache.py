...
class TestAccessGrantsCache(unittest.TestCase):
    ...
    def test_cache_hit(self):
        ...
        self.access_grants_cache._put_value_in_cache(key_1, self.access_grants_credentials)
    ...
    @mock.patch('aws_s3_access_grants_boto3_plugin.cache.access_grants_cache.AccessGrantsCache._get_credentials_from_service')
    ...
    def test_which_s3_prefixes_stored_in_cache(self):
        ...
        self.assertEqual(self.access_grants_cache._process_matched_target(grant_1),
                         "s3://bucket/foo/bar")
        self.assertEqual(self.access_grants_cache._process_matched_target(grant_2),
                         "s3://bucket/foo/bar.txt")
        self.assertEqual(self.access_grants_cache._process_matched_target(grant_3),
                         "s3:/")
        self.assertEqual(self.access_grants_cache._process_matched_target(grant_4),
                         "s3://bucket/foo*")
    ...
    def test_cache_ttl(self):
        ...
        requester_credentials = credentials.Credentials(access_key="access_key", secret_key="secret_key", token="token")
        key = CacheKey(requester_credentials, 'READ', "s3://bucket-name/prefixA")
        access_grants_cache._put_value_in_cache(key, self.access_grants_credentials)
        time.sleep(2)
